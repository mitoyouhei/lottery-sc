...
describe("Casino", function () {
  ...
  it("should create a game and transfer wager when call createGame", async function(){
    await casino.createGame(player1Bet, { value: ethers.utils.parseEther("100") });
  ...
  it("should play game and transfer wager when call playGame", async function(){
    ...
    try{
      const result = await casino.connect(player2)
        .playGame(rockPaperScissors.id, player2Bet, { value: ethers.utils.parseEther("100") });
